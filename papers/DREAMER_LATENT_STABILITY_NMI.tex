...
{\large Sehaj Randhir Singh}\\[0.2em]
{\normalsize Independent researcher; partial affiliation with NYU Tandon School
of Engineering}\\[0.2em]
{\small sehajrsinghs@gmail.com}
\end{center}
...
alignment). The study is reproducible on CPU with fixed seeds, and has been
reproduced in a single-seed run (250-step schedule, 2026-08-13): every
documented mode reappears with its own numbers (Table~\ref{tab:atlas}); the
full B/C/D/E/F summary is in the repository. The atlas is
...
github.com/sehajr-singhs/dreamerv3-latent-stability. The failure-atlas
benchmark is documented, with its measured signatures and reproduction
protocol, in a companion manuscript. All code runs on CPU.
...
\vspace{1em}
\noindent\textbf{Draft status.} References to verify; the trained-model
audit [TODO: GPU run]. The CPU reproduction of the atlas is complete and its
numbers are in this Letter. Nothing in this draft has been submitted or peer
reviewed.
