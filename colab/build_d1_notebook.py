...
cells = [
    ...
    code("import json, os, subprocess",
         ...
         "       '--burn-in', '100',",
         "       '--time-budget', '120',",
         "       '--dump-conds', '/content/rl-wm-audit/artifacts/d1_sampling_conds.npy',",
         "       '--method', 'CROWN']",
         "print(' '.join(cmd))",
    ...
    md("## 10. Paper-readiness bundle",
       "",
       "Writes everything the IEEE/NMI manuscripts reference into one place:",
       "",
       "- `results/d1_seed0.json` -- the artifact (written by the audit)",
       "- `results/paper_numbers.md` -- every measured quantity the Results sections",
       "  quote (sampling baseline, worst condition, BaB verdicts, wall reason,",
       "  reachable counterexamples, gap flag, interpretation)",
       "- `results/fig_sampling_conditions.png` -- the sampling-baseline condition",
       "  histogram (Figure for the audit record)",
       "",
       "Then download all three. Keep the JSON; quote only it."),

    code("import json, numpy as np, pathlib",
         "import matplotlib",
         "matplotlib.use('Agg')",
         "import matplotlib.pyplot as plt",
         "",
         "ROOT = pathlib.Path('/content/rl-wm-audit')",
         "res = json.load(open(ROOT / 'results/d1_seed0.json'))",
         "r = res['result']",
         "samp, bab = res['sampling'], res['bab']",
         "",
         "conds = np.load(ROOT / 'artifacts/d1_sampling_conds.npy')",
         "fig, ax = plt.subplots(figsize=(6, 3.5))",
         "ax.hist(conds, bins=80, color='#226999', alpha=0.85)",
         "ax.axvline(0, color='#c0392b', lw=1.2, ls='--')",
         "ax.set_xlabel('Lyapunov decrease condition: V(z) - V(z-prime)')",
         "ax.set_ylabel('count')",
         "ax.set_title('sampling baseline: %d/%d violations (worst cond %.2e)' %",
         "             (r['sampling_violations'], samp['n_sampled'], samp['worst_cond']))",
         "fig.tight_layout()",
         "fig.savefig(ROOT / 'results/fig_sampling_conditions.png', dpi=200)",
         "print('saved', ROOT / 'results/fig_sampling_conditions.png')",
         "",
         "md_lines = [",
         "    '# D1 audit -- paper numbers (heavy run, seed 0)',",
         "    '',",
         "    f\"- n_sampled: {samp['n_sampled']}\",",
         "    f\"- sampling violations: {samp['n_violations']} ({samp['violation_rate']:.3%})\",",
         "    f\"- worst cond: {samp['worst_cond']:.3e}\",",
         "    f\"- BaB verdicts: {bab['n_certified']} certified, {bab['n_violation']} violation,\",",
         "    f\"  {bab['n_unknown']} unknown\",",
         "    f\"- BaB wall: {bab.get('wall', 'none')}\",",
         "    f\"- reachable counterexamples: {r['reachable_counterexamples']}\",",
         "    f\"- gap_demonstrated: {r['gap_demonstrated']}\",",
         "    '',",
         "    f\"interpretation: {r['interpretation']}\",",
         "    '',",
         "    'provenance: artifacts/d1_weights_meta.json (export sidecar); verifier pinned by verify.py SHA-256.',",
         "    '',",
         "    'honest reading: unknown verdicts are category-3 verifier incompleteness, never a gap, never a finding.',",
         "]",
         "(ROOT / 'results/paper_numbers.md').write_text('\\n'.join(md_lines))",
         "print('saved', ROOT / 'results/paper_numbers.md')",
         "",
         "from google.colab import files",
         "files.download(ROOT / 'results/paper_numbers.md')",
         "files.download(ROOT / 'results/fig_sampling_conditions.png')",
         "files.download(ROOT / 'results/d1_seed0.json')"),
]
